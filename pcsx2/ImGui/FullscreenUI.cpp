...
void FullscreenUI::DrawInterfaceSettingsPage()
{
	...
	if (DrawToggleSetting(bsi, FSUI_ICONSTR(ICON_FA_LIGHTBULB, "Use Light Theme"),
			FSUI_CSTR("Uses a light colored theme instead of the default dark theme if you want to be blind."), "UI", "UseLightFullscreenUITheme", false))
	{
...
void FullscreenUI::DrawAboutWindow()
{
	...
	if (ImGui::BeginPopupModal("About XBSX2", &s_about_window_open, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize))
	{
		...
		ImGui::TextWrapped(
			...
			"combination of MIPS CPU Interpreters, Recompilers, and a Virtual Machine that manages hardware states and PS2 system memory. "
			"This allows you to play PS2 games on your Xbox via! Dev Mode, offering many additional features and benefits.");
...
TRANSLATE_NOOP("FullscreenUI", "Uses a light colored theme instead of the default dark theme.");
